dfstest <- vector()     #####take average of each city (NO2)
store <- vector()
for (i in 1:ncol(dataframes[[1]])){      
  for (j in 1:length(dataframes)){
    #if (!j == 59 && !j == 60){
    store[j] <- dataframes[[j]][8,i]        ## change the '1' if want another pollutant ~~~~~~~~~~~~~
    #}
  }
...
lm2014NO2 <- lm(dfstest[determcsv]~newxlslist2014_sim)  #~~~~~~~~~~~~~~~~NO2 2014
